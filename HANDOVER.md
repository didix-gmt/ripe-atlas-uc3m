...
- **A finding that is not yet in the docs.** By *address* count AWS is the larger half of the blocking, not Cloudflare. That does not contradict OONI, whose write-up foregrounds Cloudflare: OONI is counting affected *sites*, where Cloudflare's anycast concentration dominates. Both are true and they answer different questions, but the distinction is easy to get wrong in a write-up, and it is currently recorded only in `select_targets.py`'s header rather than in `docs/` or the SOTA. Worth promoting into the documentation proper.

...
- **Target list**: `campaigns/campaign-match/targets.txt` now holds **20 addresses sampled from OONI's published affected-address list, plus 2 canaries**, generated by `select_targets.py`. Worth knowing what it replaced: until 20 September the file was a placeholder - 3 addresses and a long comment block explaining where the other ~17 should come from. The plan in that comment turned out to be right about the method (use OONI's own published data, not guesses at Cloudflare ranges) and was simply never executed. The list has never been validated against a live match, so treat it as a well-sourced starting point, not a proven set.
- **The canaries matter and are easy to lose.** `1.1.1.1` and `8.8.8.8` are in the target list for a reason that is not obvious: they should never be blocked, so if one of them goes unreachable from Spain mid-match, the fault is in our measurement, not the network. They are a different safeguard from the out-of-country control *probes*, which answer "is this target up at all?" rather than "is our measurement working from inside Spain?". They were nearly lost when `targets.txt` was regenerated - `select_targets.py` now writes them into every list automatically, in their own labelled section.
...
**Count the canaries.** `targets.txt` as generated holds 20 sampled addresses **plus 2 canaries** = 22 targets, so the real figure for the top row is ~564,000 credits, not 513,000. The dry run reads the file and gets this right; the table above is per-target, so read off the row matching what `wc -l` actually gives you.
